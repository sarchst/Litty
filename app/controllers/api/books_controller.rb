class Api::BooksController < ApplicationController
  ...
  def index
    if params[:year].present?
      ...
      books = Book.where("EXTRACT(year FROM published_at) = ?", year).order(:year_rank, :title)
      ...
      books_by_year = books_query.order(:year_rank, :published_at).group_by { |book| book.published_at&.year }
      result = {}
      books_by_year.each do |year, books|
        sorted_books = books.sort_by { |book| [book.year_rank || Float::INFINITY, book.title || ""] }
        result[year] = sorted_books.map(&method(:book_attributes))
      end
      render json: result
    end
  end
  ...
  def book_attributes(book)
    {
      ...
      published_year: book.published_at.year,
      genres: book.genres,
